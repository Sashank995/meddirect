...
export default function HealthAssessment() {
  ...
  const validateStep = (s) => {
    if (s === 1) {
      if (!form.name.trim()) return "Please enter your name.";
      if (!form.age || Number(form.age) <= 0) return "Enter valid age.";
    }
    if (s === 2) {
      if (!form.weight || Number(form.weight) <= 0) return "Enter valid weight.";
      if (!form.height || Number(form.height) <= 0) return "Enter valid height.";
    ...
    if (s === 3) {
      if (!form.symptoms.trim()) return "Describe your symptoms.";
    }
    return null;
  };
  ...
  const submit = () => {
    ...
    setTimeout(() => {
      ...
      const recs = [];
      if (riskLevel === "High") recs.push("Consult a physician promptly.");
      if (riskLevel === "Medium") recs.push("Book a doctor's appointment soon.");
      if (riskLevel === "Low") recs.push("Self-care is appropriate for now.");
      if (form.symptoms.toLowerCase().includes("fever"))
        recs.push("If fever > 38.5°C or persistent 48+ hrs, see a doctor.");
      if (form.smoker) recs.push("Consider smoking cessation resources.");
      ...
      try {
        const existing = JSON.parse(localStorage.getItem(LS_HISTORY) || "[]");
        existing.unshift(generated);
        localStorage.setItem(LS_HISTORY, JSON.stringify(existing.slice(0, 50)));
      } catch {}
      setResult(generated);
      push("Assessment complete!", "success");
      setStep(totalSteps + 1);
    }, 1200);
  };
  ...
  const downloadPDF = () => {
    const doc = new jsPDF();
    doc.text("Health Assessment", 12, 18);
    doc.text(`Name: ${form.name}`, 12, 30);
    doc.text(`Age: ${form.age}`, 12, 38);
    doc.text(`BMI: ${result?.bmi}`, 12, 46);
    doc.text(`Risk: ${result?.riskLevel}`, 12, 54);
    (result?.recommendations || []).forEach((r, i) =>
      doc.text(`${i + 1}. ${r}`, 14, 68 + i * 8)
    );
    doc.save(`assessment_${form.name || "user"}.pdf`);
  ...
  const scheduleReminder = () => {
    push("Reminder scheduled (demo: fires in 10s).", "info");
    reminderRef.current = setTimeout(() => {
      push("⏰ Reminder: Check your recommendations.", "info");
    }, 10000);
  };
  ...
  return (
    <div className="min-h-screen bg-green-50 p-4 sm:p-6 flex flex-col items-center">
      <div className="w-full max-w-md sm:max-w-2xl lg:max-w-4xl">
        {/* Header */}
        <header className="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <div>
            <h1 className="text-xl sm:text-2xl font-bold text-green-700 flex items-center gap-2">
              <Heart className="text-red-500" /> Health Assessment
            </h1>
            <p className="text-gray-600 text-sm">
              Quick local demo assessment with multi-step wizard.
            </p>
          </div>
          <div className="flex flex-wrap gap-2">
            <button
              onClick={saveDraft}
              className="px-3 py-2 bg-white border rounded-md text-xs sm:text-sm flex items-center gap-2"
            >
              <Save className="w-4 h-4" /> Save
            </button>
            <button
              onClick={loadDraft}
              className="px-3 py-2 bg-white border rounded-md text-xs sm:text-sm"
            >
              Load
            </button>
            <button
          ...
          <div className="bg-white rounded-xl p-4 sm:p-6 shadow">
                ...
                <div className="flex flex-col sm:flex-row gap-2 sm:gap-4">
                  ...
                  <label className="inline-flex items-center gap-2">
                    <input
                      ...
                      onChange={(e) => update({ diabetic: e.target.checked })}
                    />{" "}
                    Diabetic
                  </label>
                </div>
            ...
            {step === 4 && (
              <div className="text-sm space-y-2">
                ...
                <p>
                  <strong>Weight/Height:</strong> {form.weight} / {form.height}
                </p>
                <p>
          ...
          <div className="bg-white rounded-xl p-4 sm:p-6 shadow space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
              <h2 className="text-xl font-semibold text-green-700 flex items-center gap-2">
                <CheckCircle2 className="text-green-600" /> Assessment Results
              </h2>
              <div className="flex gap-2 mt-3 sm:mt-0">
                ...
                <button
                  onClick={() => {
                    ...
                    setResult(null);
                    setStep(1);
                    push("Restarted new assessment.", "info");
                  }}
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              <div className="p-4 border rounded-lg">
                ...
                <p className="text-lg font-bold">{result.bmi || "N/A"}</p>
              </div>
              <div className="p-4 border rounded-lg">
